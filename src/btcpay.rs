...
pub async fn handle_btcpay<T: InvoiceCommands + std::clone::Clone>(
    ...
    let btcpay_sig = match req.header("BTCPAY-SIG") {
        Some(sig) => sig,
        None => return Ok(tide::Response::builder(400)
...

    {
        ...
        match db.set_invoice_status(
            update.invoice_id.clone(),
            update.status.clone(),
        ).await {
...
mod tests {
    ...
    async fn test_btcpay() {
        ...
        let update: serde_json::value::Value = json!({"invoiceId": "bob", "type": "InvoiceCreated"});
...

        {
            ...
            assert_eq!(
                invoices.get_invoice_status("bob".to_string()).await.unwrap(),
                "InvoiceCreated"
            );
        }
    }
